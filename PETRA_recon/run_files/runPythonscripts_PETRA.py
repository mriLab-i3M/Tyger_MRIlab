...
start1 = time.time()
p1 = subprocess.Popen(
    ["python3", "CP_ARTPK_ART/recon_scripts/fromMATtoMRD3D_PETRA.py", "-i", rawData],
    stdout=subprocess.PIPE,
)

# Paso 2: Código python que ejecutaré desde Tyger
yml_path = "CP_ARTPK_ART/yml_files/stream_recon_gpu_phys1_python_PETRA.yml"
...
p2 = subprocess.Popen(
    ["python3", "CP_ARTPK_ART/recon_scripts/stream_recon_PETRA.py"]+args,
    stdin=p1.stdout,
    stdout=subprocess.PIPE
)
...
# Paso 3: fromMRDtoMAT
p3 = subprocess.Popen(
    ["python3", "CP_ARTPK_ART/recon_scripts/fromMRDtoMAT3D.py", "-o", rawData, "-of", out_field],
    stdin=p2.stdout
)
p2.stdout.close()
...
plt.tight_layout()
plt.savefig('CP_ARTPK_ART/compTyger.png', bbox_inches='tight', dpi=300)
# plt.show()
