        ...
        def conjugatePhase(kx, ky, kz, x, y, z, s, rho, dBo):
            ...
            if cp_batchsize == 0:
                ...
                for i in range(0, len(x), cp_batchsize):
                    ...
                    phase_batch = np.exp(vecSign[3]*1j * (np.outer(kx, x_batch) + np.outer(ky, y_batch) + np.outer(kz, z_batch)))
                    rho[i:i+cp_batchsize] = np.dot(s, phase_batch)
