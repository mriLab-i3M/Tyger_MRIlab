...
def matToMRD(input, output_file):
    ...
    # Position vectors
    rd_pos = np.linspace(-fov_adq[0] / 2 + fov_adq[0] / (2 * nPoints[0]) , fov_adq[0] / 2 + fov_adq[0] / (2 * nPoints[0]), nPoints[0])
    ph_pos = np.linspace(-fov_adq[1] / 2 + fov_adq[1] / (2 * nPoints[1]) , fov_adq[1] / 2 + fov_adq[1] / (2 * nPoints[1]), nPoints[1])
    sl_pos = np.linspace(-fov_adq[2] / 2 + fov_adq[2] / (2 * nPoints[2]) , fov_adq[2] / 2 + fov_adq[2] / (2 * nPoints[2]), nPoints[2])
...
if __name__ == "__main__":
    ...
    # parser.set_defaults(
    #     input = '/home/teresa/marcos_tyger/Next1_10.06/RarePyPulseq.2025.06.10.13.15.36.936.mat',
    #     output= '/home/teresa/marcos_tyger/Next1_10.06/testMRDtoMAT_021.bin',
    # )
